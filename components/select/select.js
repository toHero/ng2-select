...
let select_item_1 = require('./select-item');
let select_pipes_1 = require('./select-pipes');
let common_1 = require('./common');
let off_click_1 = require('./off-click');
...
let SelectComponent = (function () {
  ...
  Object.defineProperty(SelectComponent.prototype, 'active', {
    ...
    set: function (selectedItems) {
      ...
      if (!selectedItems || selectedItems.length === 0) {
        ...
        let areItemsStrings_1 = typeof selectedItems[0] === 'string';
        this._active = selectedItems.map(function (item) {
          let data = areItemsStrings_1
                        ? item
  ...
  SelectComponent.prototype.focusToInput = function (value) {
    ...
    setTimeout(function () {
      let el = _this.element.nativeElement.querySelector('div.ui-select-container > input');
      if (el) {
        el.focus();
        el.value = value;
      }
...
let GenericBehavior = (function (_super) {
...
let ChildrenBehavior = (function (_super) {
  ...
  ChildrenBehavior.prototype.filter = function (query) {
    ...
    let startPos = 0;
    for (let _i = 0, _a = this.actor.itemObjects; _i < _a.length; _i++) {
      let si = _a[_i];
      let children = si.children.filter(function (option) { return query.test(option.text); });
